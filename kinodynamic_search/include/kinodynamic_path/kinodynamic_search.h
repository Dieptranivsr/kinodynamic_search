#ifndef _KINODYNAMIC_SEARCH_H
#define _KINODYNAMIC_SEARCH_H
...
private:
  /* data */
  std::unordered_map<Eigen::Vector3i, PathNodePtr, matrix_hash<Eigen::Vector3i>> data_3d;
  std::unordered_map<Eigen::Vector4i, PathNodePtr, matrix_hash<Eigen::Vector4i>> data_4d;
...
public:
  ...
  void insert(Eigen::Vector3i idx, int time_idx, PathNodePtr node) {
    data_4d.insert(make_pair(Eigen::Vector4i(idx(0), idx(1), idx(2), time_idx), node));
  }
  ...
  PathNodePtr find(Eigen::Vector3i idx, int time_idx) {
    auto iter = data_4d.find(Eigen::Vector4i(idx(0), idx(1), idx(2), time_idx));
    return iter == data_4d.end() ? NULL : iter->second;
  }
